...
object Currency {
  private val countryCodeToCurrencyCodeMap: SMap[String, String] = LocaleRegistry.currencydata.regions.map{ r =>
    r.countryCode -> (r.currencies.find{ _.to.isEmpty } orElse r.currencies.headOption).map{ _.currencyCode }.get
  }.toMap

  private val all: SSet[Currency] = LocaleRegistry.currencydata.currencyTypes.map{ currencyType: CurrencyType =>
    val fractions: CurrencyDataFractionsInfo = (
      LocaleRegistry.currencydata.fractions.find{ _.currencyCode == currencyType.currencyCode } orElse
      LocaleRegistry.currencydata.fractions.find{ _.currencyCode == "DEFAULT" }
    ).get

    val numericCode: Int =
      LocaleRegistry.currencydata.numericCodes.find{ _.currencyCode == currencyType.currencyCode }.map{ _.numericCode }.getOrElse(0)
