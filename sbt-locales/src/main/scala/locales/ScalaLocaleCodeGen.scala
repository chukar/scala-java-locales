...
object ScalaLocaleCodeGen {
  ...
  def constructLDMLDescriptor(
    ...
    XMLLDML(
      ...
      symbols.toMap,
      gregorian.flatten.headOption,
      gregorianDatePatterns.flatten.headOption,
      currencies,
  ...
  def buildLDMLDescriptors(
    ...
    } yield constructLDMLDescriptor(f, XML.withSAXParser(parser).load(r), latnNS, numericSystemsMap)
  }
  ...
  def generateCurrencyDataFile(base: File, currencyData: CurrencyData): File =
    writeGeneratedTree(base, "currencydata", CodeGenerator.currencyData(currencyData))
