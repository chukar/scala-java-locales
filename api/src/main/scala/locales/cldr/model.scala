...
object DateTimePattern {
  def formatIndex(p: String): Int =
    datePatternsFormat.getOrElse(p, sys.error(s"Uknown format $p"))
}
final case class CalendarPatterns(
    datePatterns: Map[Int, DateTimePattern],
    timePatterns: Map[Int, DateTimePattern]
)
